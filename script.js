//Declaring variables related to alarm clock
...
function convertTo12HourFormat(hour, minute, seconds) {
  ...
  return `${AddZeroHrs(twelveHour)}:${AddZero(minute)}:${seconds} ${ampm}`;
}
...
const createAlarm = (alarmObj) => {
  ...
  alarmDiv.innerHTML = `<span>${AddZeroHrs(displayHour)}:${alarmMinute}:${alarmSecond} ${ampm}</span>`;
...
setAlarm.addEventListener("click", () => {
  ...
  secondInput.value = AddZero(initialSecond);
